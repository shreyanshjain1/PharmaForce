...
<div class="card"><div class="section-title"><div><span class="eyebrow">Results</span><h2><?= count($reports) ?> reports found</h2></div><a class="btn ghost" href="reports.php">Reset</a></div><div class="table-wrap"><table><thead><tr><th>Date</th><th>Doctor / Hospital</th><th>Rep</th><th>Purpose</th><th>Status</th><th>Actions</th></tr></thead><tbody><?php foreach($reports as $r): ?><tr><td><?= e(date('M d, Y g:i A',strtotime($r['visit_datetime']))) ?></td><td><strong><?= e($r['doctor_name']) ?></strong><br><span class="muted"><?= e($r['hospital_name']) ?></span></td><td><?= e($r['rep']) ?></td><td><?= e($r['purpose']) ?><br><span class="muted"><?= e($r['medicine_name']) ?></span></td><td><span class="badge <?= e($r['status']) ?>"><?= e(status_label($r['status'])) ?></span></td><td><div class="actions"><a class="btn small" href="report_view.php?id=<?= (int)$r['id'] ?>">View</a><a class="btn small" href="report_form.php?id=<?= (int)$r['id'] ?>">Edit</a></div></td></tr><?php endforeach; ?></tbody></table><?php foreach($reports as $r): ?><div class="mobile-card"><strong><?= e($r['doctor_name']) ?></strong><span class="muted"><?= e($r['hospital_name']) ?></span><span class="badge <?= e($r['status']) ?>"><?= e(status_label($r['status'])) ?></span><div class="actions"><a class="btn small" href="report_view.php?id=<?= (int)$r['id'] ?>">View</a><a class="btn small" href="report_form.php?id=<?= (int)$r['id'] ?>">Edit</a></div></div><?php endforeach; ?></div></div>
<?php render_footer(); ?>
