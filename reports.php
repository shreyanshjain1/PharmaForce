<?php
require __DIR__ . '/app/bootstrap.php'; require_any_permission(['reports.view', 'reports.view_own', 'reports.view_team']);
...
<div class="card">
  ...
  <?php else: ?>
    <div class="table-wrap"><table><thead><tr><th>Visit Date</th><th>Doctor / Hospital</th><th>Rep</th><th>Purpose</th><th>Status</th><th>Saved</th><th>Actions</th></tr></thead><tbody><?php foreach($reports as $r): ?><tr><td><?= e(date('M d, Y g:i A',strtotime($r['visit_datetime']))) ?></td><td><strong><?= e($r['doctor_name']) ?></strong><br><span class="muted"><?= e($r['hospital_name']) ?></span></td><td><?= e($r['rep']) ?></td><td><?= e($r['purpose']) ?><br><span class="muted"><?= e($r['medicine_name']) ?></span></td><td><span class="badge <?= e($r['status']) ?>"><?= e(status_label($r['status'])) ?></span></td><td><strong><?= e(date('M d, Y', strtotime($r['updated_at'] ?? $r['created_at'] ?? $r['visit_datetime']))) ?></strong><br><span class="muted">Created <?= e(date('M d, Y', strtotime($r['created_at'] ?? $r['visit_datetime']))) ?></span></td><td><div class="actions"><a class="btn small" href="report_view.php?id=<?= (int)$r['id'] ?>">View</a><?php if (can('reports.edit_team') || ((int)($r['user_id'] ?? 0) === (int)(current_user()['id'] ?? 0) && can('reports.edit_own'))): ?><a class="btn small" href="report_form.php?id=<?= (int)$r['id'] ?>">Edit</a><?php endif; ?></div></td></tr><?php endforeach; ?></tbody></table><?php foreach($reports as $r): ?><div class="mobile-card"><strong><?= e($r['doctor_name']) ?></strong><span class="muted"><?= e($r['hospital_name']) ?></span><span class="badge <?= e($r['status']) ?>"><?= e(status_label($r['status'])) ?></span><span class="muted">Updated <?= e(date('M d, Y', strtotime($r['updated_at'] ?? $r['created_at'] ?? $r['visit_datetime']))) ?></span><div class="actions"><a class="btn small" href="report_view.php?id=<?= (int)$r['id'] ?>">View</a><?php if (can('reports.edit_team') || ((int)($r['user_id'] ?? 0) === (int)(current_user()['id'] ?? 0) && can('reports.edit_own'))): ?><a class="btn small" href="report_form.php?id=<?= (int)$r['id'] ?>">Edit</a><?php endif; ?></div></div><?php endforeach; ?></div>
  <?php endif; ?>
</div>
<?php render_footer(); ?>
